<template>
  <div class="h-full relative flex">
    <div class="team-cont min-w-250px shadow-md bg-white">
      <div v-for="member in myTeam">
        <div class="h-18 flex border-b-1px cursor-pointer"
          ...
          <div class="h-full min-w-12 flex items-center relative">
              <div class="h-4 w-4 rounded-full absolute bottom-0 right-0 bg-primary mb-2 border-2px border-white"
                   v-if="IsOnline($store.getters.GetStatus[member.email])"></div>
              <div class="h-4 w-4 rounded-full absolute bottom-0 right-0 bg-slate-300 mb-2 border-2px border-white" v-else></div>
    ...
    <div class="w-full h-full chat-section">
      <div class="w-full h-full  flex items-center justify-center" v-if="selectedChat === ''">
        CLick on an account and start chatting
      </div>
      <div class="h-full flex flex-col" v-else>
        <div class="header h-16 bg-slate-300 p-2 flex">
          ...
          <div class="flex flex-col ml-3">
            ...
            <div class="">{{IsOnline($store.getters.GetStatus[selectedChat.email],true)}}</div>
          </div>
        </div>
        <div class="chats-cont bg-primary h-full overflow-auto custom-scroll px-3">
          <div class="block w-full h-10 bg-orange-100 flex items-center justify-center shadow mb-5 lg:mt-10 xl:mt-16 2xl:mt-24">
            The messages are end-to-end encrypted
          </div>
          <div class="mb-3 w-full focus:bg-red-500" v-for="chat in chats[selectedChat.username]" :id="'chat-'+chat.id">
...
<!--                  <div class="absolute w-full h-10 btm-0 left-0 flex items-center bg-blue-300">-->
<!--                    {{chat.image.name}}-->
<!--                  </div>-->
                </div>
...
<!--                  </div>-->
        ...
        <div class="input-cont bg-primary-light h-16 relative w-full">
          ...
          <div class="absolute w-full min-h-16 bg-inherit bottom-full flex items-center" v-if="replyFor!=''">
          ...
          <div class="reply-msg h-16 w-full flex pt-2 justify-center items-center fixed bottom-full left-0" v-if="replyFor!=''">
            <!--            <div class="w-100px invisible"></div>-->
            <div class="w-full h-16 rounded-sm flex overflow-hidden bg-slate-300 justify-between">
              <div class="w-2 h-full bg-primary mr-2"></div>
              <div class="w-full h-full flex justify-center flex-col">
    ...
    <span>{{GetChats,WriteTeam(),GetUser(),ReadMsg,ScrollToBottom}}{{GetStatus()}}</span>
  </div>
...
export default {
  ...
  methods:{
    ...
    IsOnline(lastSeen,time){
      if(lastSeen){
        let isOnline = new Date().getTime() - lastSeen <= 90000
        return isOnline &&time?'Online':
            isOnline?true:
                time?`Last seen : ${this.DateFormat(lastSeen)}`:false
      }else{
        return time?'Last seen ':false
      }
    },
    WriteTeam(){
    ...
    GetStatus(){
      ...
      let all = this
      setInterval(()=>{
        let username = all.user==''?'':all.user.username
        if(all.myTeam == '')return
        let team = []
        let tempTeam = RemoveFromArray(Object.keys(all.myTeam),username)
        tempTeam.forEach(member=>{
          team.push(store.getters.GetTeam[member].email)
        })

        team.forEach(email=>{
          let lastSeen = store.getters.GetStatus[email]
          if(lastSeen){
            if(new Date().getTime()-lastSeen <= 90000){
              all.onlineStatus[email] = {
                onLine:true,
                lastSeen:lastSeen
              }
            }else {
        ...
        all.Writestatus(all.onlineStatus)
      },30000)
    },
  },
  computed:{
